...
- **Facebook Marketing (backend đã có):** API **`/api/shops/:shopId/facebook/...`** — đọc/ghi post qua Meta Graph (token xử lý server-side); cache post + insight (migration **006**); **AI text** (tóm tắt comment, assist caption, …) qua **Ollama** trên VPS (`MARKETING_AI_BASE_URL`). OAuth flow đầy đủ có thể bổ sung sau; xem [`aimap/backend/danhsach_API.md`](../aimap/backend/danhsach_API.md).
