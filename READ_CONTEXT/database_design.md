...
| **Payment**            | Giao dịch nạp tiền: amount_money, credits, gateway, gateway_txn_id, status, callback_data (xác nhận qua VietQR API Service callback) | P1.9, Architecture                    |
...
- **Trang Assets (UI):** Mức 1 hiển thị **danh sách kho theo shop** với **dung lượng đã dùng** và **còn trống** (backend có thể tính từ container stats hoặc object storage API theo prefix `shops/:shopId/`); mức 2 khi click vào shop → hiển thị ảnh (bảng assets) và kho content (marketing_content) của shop đó.
...
-- 6. payments (thanh toán nạp credit — VietQR + webhook provider)
